...
def calcular_determinante_auto(matriz):
    """
    Detecta automáticamente el tamaño de la matriz y aplica el método adecuado.
    Devuelve:
        {
          "metodo": str,
          "razon":  str,
    ...
    if not matriz or not isinstance(matriz, list):
        return {"metodo": "Error", "razon": "La matriz está vacía o no es válida.", "pasos": [], "resultado": None}
    n = len(matriz)
    if any(len(f) != n for f in matriz):
        return {"metodo": "Error", "razon": "La matriz no es cuadrada.", "pasos": [], "resultado": None}

    if n == 1:
        val = matriz[0][0]
        return {
            "metodo": "Trivial (1x1)",
            "razon": "El determinante de 1×1 es el único elemento.",
            "pasos": [f"det([[{formatea_num(val)}]]) = {formatea_num(val)}"],
            "resultado": val
        }

    if n == 2:
        res = _det_2x2(matriz)
        res["razon"] = "Se usa la fórmula directa ad − bc porque la matriz es 2×2."
        return res

    if n == 3:
        res = _det_3x3_sarrus(matriz)
        res["razon"] = "La Regla de Sarrus aplica exclusivamente a matrices de 3×3."
        return res

    if n <= 5:
        res = _det_cofactores(matriz)
        res["razon"] = "Para tamaños moderados (4×4 o 5×5) se prefiere cofactores para mostrar el proceso algebraico."
        return res

    res = _det_por_propiedades(matriz)
    res["razon"] = (
        "Para matrices grandes (n ≥ 6) se usa triangularización por eficiencia: "
        "operaciones de fila + producto de la diagonal, ajustando el signo por intercambios."
    )
    return res
...
def _det_2x2(m):
    ...
    a, b = m[0][0], m[0][1]
    c, d = m[1][0], m[1][1]
    pasos.append("Matriz 2×2:")
    pasos.append(matriz_a_string(m))
    pasos.append("Fórmula: det(A) = a11·a22 − a12·a21")
    pasos.append(f"Sustituyendo: det(A) = ({formatea_num(a)})·({formatea_num(d)}) − "
                 f"({formatea_num(b)})·({formatea_num(c)})")
    det = a * d - b * c
    pasos.append(f"Resultado: det(A) = {formatea_num(det)}")
    return {"metodo": "Fórmula directa (2×2)", "pasos": pasos, "resultado": det}


# ----------------------------- 3x3: Regla de Sarrus -----------------------------
def _det_3x3_sarrus(m):
    pasos = []
    a11, a12, a13 = m[0]
    a21, a22, a23 = m[1]
    a31, a32, a33 = m[2]

    pasos.append("Matriz 3×3:")
    pasos.append(matriz_a_string(m))
    pasos.append("Regla de Sarrus: suma de productos de diagonales principales − suma de diagonales secundarias.")
    # Diagonales principales (con índices)
    p1 = a11 * a22 * a33; pasos.append(f"(a11·a22·a33) = ({formatea_num(a11)})·({formatea_num(a22)})·({formatea_num(a33)}) = {formatea_num(p1)}")
    p2 = a12 * a23 * a31; pasos.append(f"(a12·a23·a31) = ({formatea_num(a12)})·({formatea_num(a23)})·({formatea_num(a31)}) = {formatea_num(p2)}")
    p3 = a13 * a21 * a32; pasos.append(f"(a13·a21·a32) = ({formatea_num(a13)})·({formatea_num(a21)})·({formatea_num(a32)}) = {formatea_num(p3)}")
    suma_principales = p1 + p2 + p3
    pasos.append(f"Suma diagonales principales = {formatea_num(suma_principales)}")

    # Diagonales secundarias (con índices)
    s1 = a13 * a22 * a31; pasos.append(f"(a13·a22·a31) = ({formatea_num(a13)})·({formatea_num(a22)})·({formatea_num(a31)}) = {formatea_num(s1)}")
    s2 = a11 * a23 * a32; pasos.append(f"(a11·a23·a32) = ({formatea_num(a11)})·({formatea_num(a23)})·({formatea_num(a32)}) = {formatea_num(s2)}")
    s3 = a12 * a21 * a33; pasos.append(f"(a12·a21·a33) = ({formatea_num(a12)})·({formatea_num(a21)})·({formatea_num(a33)}) = {formatea_num(s3)}")
    suma_secundarias = s1 + s2 + s3
    pasos.append(f"Suma diagonales secundarias = {formatea_num(suma_secundarias)}")

    det = suma_principales - suma_secundarias
    pasos.append(f"Determinante = {formatea_num(suma_principales)} − {formatea_num(suma_secundarias)} = {formatea_num(det)}")

    return {"metodo": "Regla de Sarrus (3×3)", "pasos": pasos, "resultado": det}


# ----------------------------- Cofactores (4×4–5×5) -----------------------------
def _det_cofactores(matriz):
    """
    Expansión por cofactores con pasos detallados e indentación por nivel.
    Por defecto, expande por la FILA 1 (índice 0).
    """
    def minor(A, i, j):
        return [fila[:j] + fila[j+1:] for idx, fila in enumerate(A) if idx != i]

    def det_rec(A, depth=0):
        n = len(A)
        ind = "  " * depth
        pasos_loc = []
        pasos_loc.append(f"{ind}Submatriz ({n}×{n}):")
        pasos_loc.append(ind + matriz_a_string(A))

        if n == 1:
            val = A[0][0]
            pasos_loc.append(f"{ind}det = {formatea_num(val)} (1×1)")
            return val, pasos_loc

        if n == 2:
            a, b = A[0][0], A[0][1]
            c, d = A[1][0], A[1][1]
            det2 = a * d - b * c
            pasos_loc.append(f"{ind}det(2×2) = a11·a22 − a12·a21 = "
                             f"{formatea_num(a)}·{formatea_num(d)} − {formatea_num(b)}·{formatea_num(c)} = {formatea_num(det2)}")
            return det2, pasos_loc

        # Expansión por la fila 1 (i = 0)
        i = 0
        total = 0.0
        terminos = []
        for j in range(n):
            aij = A[i][j]
            signo = (-1) ** (i + j)
            C_ij_signo = "+" if signo > 0 else "-"
            pasos_loc.append(f"{ind}Elemento a1{j+1} = {formatea_num(aij)}; signo = {C_ij_signo} (por (−1)^(1+{j+1}))")

            M = minor(A, i, j)
            sub_det, sub_pasos = det_rec(M, depth+1)
            termino = signo * aij * sub_det
            terminos.append(termino)

            pasos_loc.extend(sub_pasos)
            pasos_loc.append(f"{ind}Término: {C_ij_signo} {formatea_num(abs(aij))} × det(M1{j+1}) = {C_ij_signo} {formatea_num(abs(aij * sub_det))}")
            total += termino

        suma_str = " + ".join(formatea_num(t) for t in terminos[:-1]) + ((" + " if terminos[:-1] else "") + formatea_num(terminos[-1]))
        pasos_loc.append(f"{ind}Suma de términos = {suma_str} = {formatea_num(total)}")
        return total, pasos_loc

    det, pasos = det_rec(copia_profunda(matriz))
    return {"metodo": "Expansión por Cofactores (4×4–5×5)", "pasos": pasos, "resultado": det}
...
def resolver_cramer(A, b):
    ...
    Devuelve los pasos explicativos y la solución.
    """
    pasos = []
    n = len(A)
    ...
    detA = detA_data["valor"] if isinstance(detA_data, dict) and "valor" in detA_data else (
        detA_data.get("resultado") if isinstance(detA_data, dict) and "resultado" in detA_data else (
        detA_data.get("determinante") if isinstance(detA_data, dict) and "determinante" in detA_data else detA_data))
    ...
    for i in range(n):
        ...
        detAi = detAi_data["valor"] if isinstance(detAi_data, dict) and "valor" in detAi_data else (
            detAi_data.get("resultado") if isinstance(detAi_data, dict) and "resultado" in detAi_data else (
            detAi_data.get("determinante") if isinstance(detAi_data, dict) and "determinante" in detAi_data else detAi_data))

        xi = detAi / detA

        pasos.append(f"\nMatriz A{i+1}(b):\n{matriz_a_string(Ai)}")
        pasos.append(f"det(A{i+1}(b)) = {formatea_num(detAi)}")
        pasos.append(f"x{i+1} = det(A{i+1}(b)) / det(A) = {formatea_num(detAi)} / {formatea_num(detA)} = {formatea_num(xi)}")

        soluciones.append(formatea_num(xi))

    pasos.append("\n=== SOLUCIÓN FINAL ===")
    for i, val in enumerate(soluciones):
        pasos.append(f"x{i+1} = {val}")
