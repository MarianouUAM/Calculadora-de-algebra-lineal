...
class CalculadoraApp:
    def __init__(self, root):
        self.root = root
        self.root.title("MatrixScholar — Proyecto Universitario")
    ...
    def _construir_shell(self):
        ...
        topbar_left = tk.Frame(topbar, bg="#242424")
        topbar_left.pack(side="left", fill="x", expand=True)
        topbar_right = tk.Frame(topbar, bg="#242424")
        topbar_right.pack(side="right", padx=20)

        # --- Título del proyecto ---
        tk.Label(topbar_left, text="MatrixScholar — Proyecto Universitario",
                bg="#242424", fg=self.COLOR_PRIMARY,
                font=("Segoe UI", 16, "bold")).pack(anchor="w", padx=20, pady=12)

        # ====== BARRA DE BÚSQUEDA CON AUTOCOMPLETADO ======
        self.search_var = tk.StringVar()
        search_frame = tk.Frame(topbar_right, bg="#242424")
        ...
        # Efecto de borde neón al enfocar
        def on_focus_in(event):
            search_entry.config(highlightbackground="#00C8FF", highlightcolor="#00C8FF", highlightthickness=1)
        ...
        self.suggestion_box.place_forget()  # Oculto por defecto
        ...
        footer = tk.Frame(self.root, bg=self.COLOR_BG, height=10)
    ...
    def _construir_sidebar(self):
        """Sidebar ancho fijo con espaciado mejorado."""
        def crear_boton_nav(parent, texto, comando, icono="•", hover_color=None):
            btn = tk.Frame(parent, bg="#252525", height=44)
            btn.pack(fill="x", pady=3)
            btn.pack_propagate(False)

            cont = tk.Frame(btn, bg="#252525")
            cont.pack(fill="both", expand=True, padx=25)

            lbl_icon = tk.Label(cont, text=icono, fg=self.COLOR_PRIMARY, bg="#252525", font=("Segoe UI", 12))
            lbl_icon.pack(side="left", padx=(0, 6))

            lbl_text = tk.Label(cont, text=texto, fg=self.COLOR_TEXT, bg="#252525",
                                font=("Segoe UI", 11), anchor="w", cursor="hand2")
            lbl_text.pack(side="left", fill="x", expand=True)

            def on_enter(e):
                cont.config(bg=hover_color if hover_color else self.COLOR_PRIMARY)
                lbl_icon.config(bg=cont["bg"], fg="black" if not hover_color else "white")
                lbl_text.config(bg=cont["bg"], fg="black" if not hover_color else "white")

            def on_leave(e):
                cont.config(bg="#252525")
                lbl_icon.config(bg="#252525", fg=self.COLOR_PRIMARY)
                lbl_text.config(bg="#252525", fg=self.COLOR_TEXT)

            for w in (cont, lbl_icon, lbl_text):
        ...
        self.sidebar.config(width=320)

        header = tk.Frame(self.sidebar, bg="#1a1a1a", height=90)
        header.pack(fill="x")
        ttk.Label(header, text="HERRAMIENTAS", style="Muted.TLabel").pack(anchor="w", padx=25, pady=(18, 2))
        ttk.Label(header, text="Álgebra Lineal", style="H2.TLabel").pack(anchor="w", padx=25)
        tk.Frame(self.sidebar, bg="#303030", height=2).pack(fill="x", pady=(5, 8))

        crear_boton_nav(self.sidebar, "Inicio", self.vista_menu, "🏠")

        def crear_acordeon(titulo, items):
            acordeon = tk.Frame(self.sidebar, bg="#252525")
            acordeon.pack(fill="x", pady=(10, 0))
            header = tk.Frame(acordeon, bg="#2c2c2c", height=35)
            header.pack(fill="x")
            lbl_titulo = tk.Label(header, text=f"  {titulo}", bg="#2c2c2c", fg=self.COLOR_SUB,
                                font=("Segoe UI", 9, "bold"), anchor="w")
            lbl_titulo.pack(side="left", fill="x", expand=True, padx=10)
            icon = tk.Label(header, text="▼", bg="#2c2c2c", fg=self.COLOR_PRIMARY, font=("Segoe UI", 10))
            icon.pack(side="right", padx=10)
            contenido = tk.Frame(acordeon, bg="#252525")
            abierto = tk.BooleanVar(value=False)
            def toggle():
                if abierto.get():
                    ...
                    icon.config(text="▲")
                    abierto.set(True)
            for w in (header, lbl_titulo, icon):
                w.bind("<Button-1>", lambda e: toggle())
            for texto, comando, icono in items:
                crear_boton_nav(contenido, texto, comando, icono)

        crear_acordeon("SISTEMAS LINEALES", [
            ("Gauss-Jordan", lambda: self.preparar_matriz_unica("Gauss-Jordan"), "⚡"),
        ...
        crear_acordeon("OPERACIONES MATRICIALES", [
        ...
        crear_acordeon("VECTORES Y TRANSFORMACIONES", [
            ("Independencia Lineal", self.preparar_vectores_independencia, "📊"),
            ("Probar Linealidad", self.preparar_linealidad, "📈"),
        ])
        
        crear_acordeon("MÉTODOS NUMÉRICOS", [
        ("Método de Bisección", self.abrir_biseccion, "📉"),
        ])


        tk.Frame(self.sidebar, bg="#303030", height=1).pack(fill="x", pady=(15, 10))
        crear_boton_nav(self.sidebar, "Cerrar Aplicación", lambda: self.root.destroy(), "🚪", hover_color="#D9534F")

        version = tk.Label(self.sidebar, text="v1.0 • Proyecto Académico",
                        bg="#252525", fg="#7f7f7f", font=("Segoe UI", 8))
 ...
 # ================================================================
    ...
    def abrir_biseccion(self):
        """Interfaz del Método de Bisección con mejoras visuales y sin 'math.' predeterminado."""
        self.clear_content()
        ttk.Label(self.content, text="Método de Bisección", style="H1.TLabel").pack(anchor="w", padx=22, pady=(18, 8))

        # --- Card de parámetros ---
        card = self.card()
        ttk.Label(card, text="Ingrese los parámetros de la función", style="CardTitle.TLabel").pack(anchor="center", pady=(10, 15))

        frame = tk.Frame(card, bg=self.COLOR_CARD)
        frame.pack(pady=10)

        # --- Entradas de función e intervalos ---
        tk.Label(frame, text="f(x) =", bg=self.COLOR_CARD, fg=self.COLOR_TEXT, font=("Segoe UI", 12)).grid(row=0, column=0, padx=5, pady=5)
        self.funcion_var = tk.StringVar(value="cos(x) - x")  # más natural, sin 'math.'
        entry_func = tk.Entry(frame, textvariable=self.funcion_var, width=40, font=("Consolas", 12),
                            bg="#3a3a3a", fg=self.COLOR_TEXT, insertbackground=self.COLOR_PRIMARY, relief="flat")
        entry_func.grid(row=0, column=1, columnspan=4, padx=5, pady=5)

        tk.Label(frame, text="a:", bg=self.COLOR_CARD, fg=self.COLOR_TEXT, font=("Segoe UI", 12)).grid(row=1, column=0, pady=5)
        self.a_var = tk.DoubleVar(value=0)
        tk.Entry(frame, textvariable=self.a_var, width=10, font=("Consolas", 11),
                bg="#3a3a3a", fg=self.COLOR_TEXT, insertbackground=self.COLOR_PRIMARY, relief="flat").grid(row=1, column=1)

        tk.Label(frame, text="b:", bg=self.COLOR_CARD, fg=self.COLOR_TEXT, font=("Segoe UI", 12)).grid(row=1, column=2)
        self.b_var = tk.DoubleVar(value=1)
        tk.Entry(frame, textvariable=self.b_var, width=10, font=("Consolas", 11),
                bg="#3a3a3a", fg=self.COLOR_TEXT, insertbackground=self.COLOR_PRIMARY, relief="flat").grid(row=1, column=3)

        tk.Label(frame, text="ε:", bg=self.COLOR_CARD, fg=self.COLOR_TEXT, font=("Segoe UI", 12)).grid(row=1, column=4)
        self.tol_var = tk.DoubleVar(value=0.0001)
        tk.Entry(frame, textvariable=self.tol_var, width=10, font=("Consolas", 11),
                bg="#3a3a3a", fg=self.COLOR_TEXT, insertbackground=self.COLOR_PRIMARY, relief="flat").grid(row=1, column=5)

        # --- Área de procedimiento ---
        proc_card = self.card()
        ttk.Label(proc_card, text="📋 Procedimiento:", style="CardTitle.TLabel").pack(anchor="w", padx=10, pady=(5, 3))
        self.txt_proc = tk.Text(proc_card, height=22, bg="#1e1e1e", fg="#66fcf1",
                                insertbackground="#66fcf1", font=("Consolas", 11), wrap="none")
        self.txt_proc.pack(fill="both", expand=True, padx=12, pady=5)
        self.txt_proc.config(state="disabled")

        # --- Resultado integrado (estilo tarjeta visual) ---
        res_card = self.card()
        ttk.Label(res_card, text="✅ Resultado:", style="CardTitle.TLabel").pack(anchor="w", padx=10, pady=(4, 0))

        res_frame = tk.Frame(res_card, bg="#252525", relief="flat")
        res_frame.pack(fill="x", padx=12, pady=8)

        self.resultado_var = tk.StringVar(value="")
        resultado_lbl = tk.Label(res_frame, textvariable=self.resultado_var, bg="#252525",
                                fg="#00ffcc", font=("Consolas", 12, "bold"), anchor="w", justify="left")
        resultado_lbl.pack(fill="x", padx=10, pady=5)

        # --- Botones ---
        btn_card = self.card()
        ttk.Button(btn_card, text="Calcular", style="Primary.TButton", command=self._calcular_biseccion).pack(side="left", padx=8)
        ttk.Button(btn_card, text="Limpiar", style="Accent.TButton", command=self._limpiar_biseccion).pack(side="left", padx=8)
        ttk.Button(btn_card, text="Volver", style="Danger.TButton", command=self.vista_menu).pack(side="left", padx=8)

    def _f_biseccion(self, x):
        """Evalúa f(x) permitiendo escribir funciones sin 'math.' (ej: ln, sin, exp, sqrt)."""
        try:
            ...
            return eval(self.funcion_var.get(), {"__builtins__": None, **funciones, "x": x})
        except Exception as e:
            raise ValueError(f"Error al evaluar la función: {e}")


    def _calcular_biseccion(self):
        """Ejecuta el método de bisección mostrando una tabla igual a Excel."""
        try:
            ...
            # === Preparar interfaz ===
            self.txt_proc.config(state="normal")
            self.txt_proc.delete("1.0", "end")

            header = f"{'Iter':<5}{'a':>12}{'b':>12}{'c':>12}{'f(a)':>12}{'f(b)':>12}{'f(c)':>12}\n"
            self.txt_proc.insert("end", header)
            self.txt_proc.insert("end", "-" * 80 + "\n")
            ...
            while error > tol:
                ...
                linea = f"{iteracion:<5}{a:>12.6f}{b:>12.6f}{c:>12.6f}{fa:>12.6f}{fb:>12.6f}{fc:>12.6f}\n"
                self.txt_proc.insert("end", linea)
            ...
            self.txt_proc.insert("end", "-" * 80 + "\n")
            self.txt_proc.insert("end", f"Se realizó en {iteracion - 1} iteraciones\n")
            self.txt_proc.insert("end", f"x = {c:.8f}\n")
            self.txt_proc.insert("end", f"Error = {error:.8f}\n")

            self.txt_proc.config(state="disabled")
            self.resultado_var.set(f"x ≈ {c:.8f}  |  Iteraciones: {iteracion - 1}  |  Error: {error:.8f}")
    ...
    def resolver_cramer_visual(self):
        ...
        # Usar convertir_valor desde logica_calculadora para evitar problemas de alcance
        ...
        # ======== RESULTADOS EN DECIMAL Y FRACCIÓN ========
        # Tomar las claves que devuelve resolver_cramer
        ...
        # MOSTRAR DECIMALES (si existen)
        if sol_decimal:
            sol_decimal_card = self.card()
            ttk.Label(sol_decimal_card, text="Solución en Decimal",
                    style="CardTitle.TLabel").pack(anchor="w", padx=15, pady=(5, 3))

            for i, decimal in enumerate(sol_decimal):
                texto_sol = f"x{i+1} = {decimal}"
                ttk.Label(sol_decimal_card, text=texto_sol,
                        style="CardText.TLabel", foreground="#4CAF50",
                        font=("Consolas", 12, "bold")).pack(anchor="w", padx=20, pady=2)

        # MOSTRAR FRACCIONES (si existen) — ESTA PARTE ES LA CLAVE
        if sol_fraccion:
            sol_fraccion_card = self.card()
            ttk.Label(sol_fraccion_card, text="Solución en Fracciones",
                    style="CardTitle.TLabel").pack(anchor="w", padx=15, pady=(5, 3))

            for i, fraccion in enumerate(sol_fraccion):
                texto_frac = f"x{i+1} = {fraccion}"
                ttk.Label(sol_fraccion_card, text=texto_frac,
