...
class CalculadoraApp:
    ...
    def resolver_cramer_visual(self):
        """Lee las entradas y llama a la lógica de Cramer."""
        try:
            A = [[float(e.get()) for e in fila] for fila in self.matriz_A]
            b = [float(self.vector_b[i][0].get()) for i in range(len(self.vector_b))]
        except ValueError:
        ...
        self.clear_content()
        ttk.Label(self.content, text="Resultado — Regla de Cramer", style="H1.TLabel").pack(anchor="w", padx=22, pady=(18, 8))

        pasos_card = self.card()
        ttk.Label(pasos_card, text="Pasos del método", style="CardTitle.TLabel").pack(anchor="w", padx=15, pady=(5, 3))
        ...
        if resultado["solucion"]:
            sol_card = self.card()
            ttk.Label(sol_card, text="Solución", style="CardTitle.TLabel").pack(anchor="w", padx=15, pady=(5, 3))
            for i, val in enumerate(resultado["solucion"]):
                ttk.Label(sol_card, text=f"x{i+1} = {val}", style="CardText.TLabel").pack(anchor="w", padx=20)
        else:
            ttk.Label(self.content, text="El sistema no tiene solución única.", style="CardText.TLabel").pack(anchor="w", padx=22, pady=10)
    ...
    def resolver_determinante_auto(self):
        """Detecta automáticamente el método y muestra pasos explicativos."""
        from logica_calculadora import calcular_determinante_auto

        try:
            matriz = [[float(e.get()) for e in fila] for fila in self.matriz_det]
        except ValueError:
            messagebox.showerror("Error", "Solo se permiten números.")
            return
        ...
        self.clear_content()
        ttk.Label(
            self.content, text="Resultado — Determinante de una Matriz",
            style="H1.TLabel"
        ).pack(anchor="w", padx=22, pady=(18, 8))

        # Mostrar explicación del método usado
        info_card = self.card()
        ttk.Label(info_card, text=f"Método aplicado: {resultado['metodo']}", style="CardTitle.TLabel").pack(anchor="w", padx=15, pady=(5, 3))
        ...
        res_card = self.card()
        ttk.Label(res_card, text=f"Determinante final: {formatea_num(resultado['resultado'])}", style="CardTitle.TLabel").pack(anchor="center", pady=10)

        ttk.Button(
            self.content, text="Volver al menú principal",
